...
async function AddProductHandler(){
  ...
  if (!titleRef.current.value.trim().length) {
    return toast.error(`Maxsulotning nomini kiriting`);
   } else if (!companyRef.current.value.trim().length) {
     return toast.error(`Maxsulotning brendini kiriting`);
   }else if (!countRef.current.value.trim().length) {
     return toast.error(`Maxsulotning sonini kiriting`);
   }
   else if (!priceRef.current.value.trim().length) {
     return toast.error(`Maxsulotning narxini kiriting`);
   }
   else if (!categoryRef.current.value.trim().length) {
     return toast.error(`Maxsulotning turini kiriting`);
   }
   else if (!img.trim().length) {
     return toast.error(`Maxsulot uchun rasm yuklang`);
   }
   
   try {
     const {data} = await axios.post(`${BACKEND_URL}/products/add-product`, {
        ...
        <div className="main_wrp p-2 px-5 mt-3 flex gap-4">
          <div className="left w-[45%] flex flex-column gap-3">
            <label className="flex flex-column gap-2 item">
              Nomi *
              <input placeholder="Whey Protein" ref={titleRef} />
            </label>
            <label className="flex flex-column gap-2 item">
              Brand *
              <input placeholder="PureGold" ref={companyRef} />
            </label>
            <label className="flex flex-column gap-2 item">
              Ma'lumot *
              <textarea ref={descRef} placeholder="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. "></textarea>
            </label>
            <label className="flex flex-column gap-2 item">
              Soni *
          ...
          <div className="right w-[45%] flex flex-column gap-3">
            <label className="flex flex-column gap-2 item">
              Narxi *
              <input ref={priceRef} placeholder="15" type="number" />
            </label>
            <label className="flex flex-column gap-2 item">
              Turi *
              <input  placeholder="Runner" ref={categoryRef} />
            </label>
            <label className="flex flex-column gap-2 item">
              rasmi *
...
click here to select file`
        ...
        <div className="bottom">
          <div className="btns flex w-[25%] justify-between items-center">
            ...
            <button className="add" onClick={AddProductHandler}>qo'shish</button>
          </div>
        </div>
      </section>
    </div>
